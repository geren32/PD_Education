function associations(sequelize) {
    const { dealer, user, manager_sr, region_activity, client, product, product_category,
        product_to_attribute, product_to_category, product_variations, booking, booking_attachment,
        payment, address, orders, attribute, model, brand, manufacturer, posts, post_to_category,
        post_category , manager_blum, position_activity, activity, product_kit_category, pages,
        product_to_kit, product_kit, change_dealer_request ,attribute_kit,
        product_category_kit ,product_kit_to_attribute, sr_manager_to_regions,change_data_request, cart, order_kits,
        phone_numbers, uploaded_images, posts_body, posts_body_to_uploaded_images, booking_history, booking_revision,
        forms, form_comments } = sequelize.models;




    user.hasOne(dealer,{ foreignKey: 'user_id', sourceKey: 'id'});
    dealer.belongsTo(user, { foreignKey: 'user_id', sourceKey: 'id'});
